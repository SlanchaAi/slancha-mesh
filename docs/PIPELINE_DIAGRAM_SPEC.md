...
| `models_local` | **Local Models** — 2× vLLM on d325 (post-swap 2026-05-17) | wide rounded rects | right side of proxy | `qwen3-coder-30b` (vLLM :8002, FP8, coder specialist) + `qwen3-8b` (vLLM :8003, FP16, generalist) | Box glows when active; intensity ∝ recent invocations |
| `models_cloud` | **Cloud OSS** — OpenRouter fan-out | rounded rect | right side of proxy, below local | olmo-3, Llama-3.1, Mixtral, DeepSeek (cost-bounded fallback) | Glow when fallback fires |
